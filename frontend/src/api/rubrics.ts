...
export async function getRubrics(params?: { scenario_type?: string }) {
  const { data } = await apiClient.get<Rubric[]>("/scoring/rubrics", {
    params,
  });
  return data;
}
...
export async function createRubric(payload: RubricCreate) {
  const { data } = await apiClient.post<Rubric>("/scoring/rubrics", payload);
  return data;
}
...
export async function deleteRubric(id: string) {
  await apiClient.delete(`/scoring/rubrics/${id}`);
}
