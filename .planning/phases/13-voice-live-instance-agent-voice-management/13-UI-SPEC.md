...
| Slate tint | `bg-slate-100 text-slate-700` | Basic tier model badge background |
...
| VoiceLiveModelSelect | `frontend/src/components/admin/voice-live-model-select.tsx` | Grouped Select dropdown for Voice Live generative AI model selection. Groups: Pro tier, Basic tier, Lite tier. Each group has a non-selectable group label. Each option shows model name. Uses existing Select/SelectContent/SelectItem components with `SelectGroup` and `SelectLabel` for tier grouping. Default: `"gpt-4o"`. |
...
| Model tier: Basic | `admin:hcp.modelTierBasic` | Basic | 基础版 |
